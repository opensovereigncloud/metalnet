...
// DeepCopyInto overrides the default DeepCopyInto to ensure arrays are sorted
func (in *NetworkInterface) DeepCopyInto(out *NetworkInterface) {
	*out = *in
	...
	// Deep copy Spec
	out.Spec = NetworkInterfaceSpec{
		NetworkRef: in.Spec.NetworkRef,
		NodeName:   in.Spec.NodeName,
		VirtualIP:  in.Spec.VirtualIP.DeepCopy(),
		NAT:        in.Spec.NAT.DeepCopy(),
	}

	// Copy and sort IPFamilies
	...
	if in.Spec.Prefixes != nil {
		out.Spec.Prefixes = make([]IPPrefix, len(in.Spec.Prefixes))
		copy(out.Spec.Prefixes, in.Spec.Prefixes)
		SortIPPrefixes(out.Spec.Prefixes)
	}

	// Copy and sort LoadBalancerTargets
	if in.Spec.LoadBalancerTargets != nil {
		out.Spec.LoadBalancerTargets = make([]IPPrefix, len(in.Spec.LoadBalancerTargets))
		copy(out.Spec.LoadBalancerTargets, in.Spec.LoadBalancerTargets)
		SortIPPrefixes(out.Spec.LoadBalancerTargets)
	}

	// Copy FirewallRules
	if in.Spec.FirewallRules != nil {
		out.Spec.FirewallRules = make([]FirewallRuleSpec, len(in.Spec.FirewallRules))
	...
	if out.Status.Prefixes != nil {
		SortIPPrefixes(out.Status.Prefixes)
	}
	if out.Status.LoadBalancerTargets != nil {
		SortIPPrefixes(out.Status.LoadBalancerTargets)
	}
}
...
// SetLoadBalancerTargets sets and sorts the LoadBalancerTargets to ensure consistent ordering
func (n *NetworkInterfaceSpec) SetLoadBalancerTargets(targets []IPPrefix) {
	n.LoadBalancerTargets = targets
	SortIPPrefixes(n.LoadBalancerTargets)
}

// GetLoadBalancerTargets returns a sorted copy of LoadBalancerTargets
func (n *NetworkInterfaceSpec) GetLoadBalancerTargets() []IPPrefix {
	if len(n.LoadBalancerTargets) == 0 {
		return nil
	}
	targets := make([]IPPrefix, len(n.LoadBalancerTargets))
	copy(targets, n.LoadBalancerTargets)
	SortIPPrefixes(targets)
	return targets
}

// SetPrefixes sets and sorts the Prefixes to ensure consistent ordering
func (n *NetworkInterfaceSpec) SetPrefixes(prefixes []IPPrefix) {
	n.Prefixes = prefixes
	SortIPPrefixes(n.Prefixes)
}

// GetPrefixes returns a sorted copy of Prefixes
func (n *NetworkInterfaceSpec) GetPrefixes() []IPPrefix {
	if len(n.Prefixes) == 0 {
		return nil
	}
	prefixes := make([]IPPrefix, len(n.Prefixes))
...
func (n *NetworkInterfaceStatus) SetLoadBalancerTargets(targets []IPPrefix) {
	n.LoadBalancerTargets = targets
	SortIPPrefixes(n.LoadBalancerTargets)
}

func (n *NetworkInterfaceStatus) GetLoadBalancerTargets() []IPPrefix {
	if len(n.LoadBalancerTargets) == 0 {
		return nil
	}
	targets := make([]IPPrefix, len(n.LoadBalancerTargets))
...
func (n *NetworkInterfaceStatus) SetPrefixes(prefixes []IPPrefix) {
	n.Prefixes = prefixes
	SortIPPrefixes(n.Prefixes)
}

func (n *NetworkInterfaceStatus) GetPrefixes() []IPPrefix {
	if len(n.Prefixes) == 0 {
		return nil
	}
	prefixes := make([]IPPrefix, len(n.Prefixes))
